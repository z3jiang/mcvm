...
Raphael.fn.connection = function (obj1, obj2, style) {
  ...
  var edge = {/*
    ...
    drawSelfEdge: function()
    {
      ...
      var p = {x:bb.x + bb.width/2, y: bb.y};

      // two curve points
      var c1 = {x:p.x - xoff, y:p.y + yoff};
      var c2 = {x:p.x + xoff, y:p.y + yoff};
      ...
      path = path.join(', ');
      console.log('self edge path: ' + path);

      // copy & paste from drawEdge
      ...
      {
        var lloc = {x: (c1.x+c2.x)/2 - style.label.length/2, y: c1.y};

        if (edge.label)
        {
          edge.label.attr(lloc) 
        }
        else
        {
          edge.label = selfRef.text(lloc.x, lloc.y, style.label).attr(
              {fill: "#000", "font-size": style["font-size"] || "12px"});
        }

        style["label-style"] && edge.label && edge.label.attr(style["label-style"]);
        
        style.callback && style.callback(edge);
      }
