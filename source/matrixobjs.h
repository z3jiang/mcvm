...
public:
        ...
        virtual ~MatrixObj() { std::cout << "cleaning base mastrix" << std::endl ; } ;
	...
	virtual MatrixObj* getSliceND(const ArrayObj* pSlice) const
	{
		...
		{		
			...
			if (pCurSlice->isMatrixObj())
			{
				...
				if (pCurSlice->getType() == DataObject::Type::LOGICALARRAY)
				{
			...
			else if (pCurSlice->getType() == DataObject::Type::RANGE)
			{
...
void getSliceND(const ArrayObj* pSlice, size_t curDim, size_t* pIndices, ScalarType*& pDstElem) const
	{		
		...
		{
			// If the object is a matrix
			if (pCurSlice->getType() == DataObject::Type::MATRIX_F64)
			{
				// Get a typed pointer to the matrix object
			...
			// If the object is a logical array
			else if (pCurSlice->getType() == DataObject::Type::LOGICALARRAY)
			{
				// Get a typed pointer to the matrix object
				MatrixObj<bool>* pMatrix = (MatrixObj<bool>*)pCurSlice;
		...
		else
		{
          ...
          if (getType() == DataObject::Type::STRUCTARRAY) {
            typedef std::map<std::string,DataObject*> ScalarStruct;
            ScalarStruct** pSrcAddrStruct = reinterpret_cast<ScalarStruct**> (pSrcAddr);
            if (*pSrcAddrStruct == NULL)
              *pSrcAddrStruct = new ScalarStruct();
          }
...
template <> inline DataObject::Type MatrixObj<float32>::getClassType() 
{ return DataObject::Type::MATRIX_F32; 	}

template <> inline DataObject::Type MatrixObj<float64>::getClassType() 
{ return DataObject::Type::MATRIX_F64; 	}

template <> inline DataObject::Type MatrixObj<Complex128>::getClassType() 
{ return DataObject::Type::MATRIX_C128; 	}

template <> inline DataObject::Type MatrixObj<bool>::getClassType() 
{ return DataObject::Type::LOGICALARRAY;	}

template <> inline DataObject::Type MatrixObj<char>::getClassType() 
{ return DataObject::Type::CHARARRAY;  	}

template <> inline DataObject::Type MatrixObj<DataObject*>::getClassType() 
{ return DataObject::Type::CELLARRAY;  	}

//template <> inline DataObject::Type MatrixObj<ScalarStruct*>::getClassType() { return DataObject::Type::STRUCTARRAY;  	}
