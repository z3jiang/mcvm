...
void TypeInfo::operator<< (const TypeInfo& typeinfo) {
    ...
    if ( this->getObjType() != DataObject::Type::STRUCTARRAY ||
            typeinfo.getObjType() != DataObject::Type::STRUCTARRAY ) {
        return ;
    }


    // Merge the size first
    if (typeinfo.getSizeKnown()) {
        auto typeinfo_size = typeinfo.getMatSize() ;
        int current_dim_number = m_matSize.size() ;
                ...
                ++it,++id) 
        {
            if (id > current_dim_number) {
                ...
                m_matSize.at(id) = typeinfo_size.at(id);
            } else {
                // Take the max
            m_matSize.at(id) = 
                std::max(
    ...
    for (auto it = std::begin(fields),
            ...
            ++it)
    {
        auto field_exist = m_fields.find (it->first) ;

        if (field_exist == std::end(m_fields)) {
            m_fields.insert(*it) ;
        } else {
            // Recursive merge of this field
            *field_exist->second << *it->second ;
        }
    }
    // Done
}
...
*/
TypeInfo::TypeInfo()
: m_objType(DataObject::Type::MATRIX_F64),
  m_is2D(false),
  m_isScalar(false),
  m_isInteger(false),
  m_sizeKnown(false),
  ...
  m_pFunction(NULL)
{
    ...
    if (pObject->isMatrixObj())
    {
        ...
        if (pMatrixObj->isScalar())
        {
            ...
            if (pMatrixObj->getType() == DataObject::Type::MATRIX_F64)
            {
        ...
        else
        {
            ...
            if (storeMatDims)
            {
                ...
                if (pMatrixObj->isEmpty() && pObject->getType() != DataObject::Type::CELLARRAY)
                {
            ...
            if (pMatrixObj->getType() == DataObject::Type::MATRIX_F64 && scanMatrices)
            {
                // Get a typed pointer to the matrix
        ...
        if (pObject->getType() == DataObject::Type::LOGICALARRAY || pObject->getType() == DataObject::Type::CHARARRAY)
        {
            // Set the integer flag
            m_isInteger = true;
        }
        ...
        else if (pObject->getType() == DataObject::Type::CELLARRAY && scanMatrices)
        {
    ...
    else if (pObject->getType() == DataObject::Type::STRUCTARRAY) {
      assert(false) ;
    }
    }

    // Otherwise, if the object is a function handle
    else if (pObject->getType() == DataObject::Type::FN_HANDLE)
    {
...
std::string TypeInfo::toString() const
{
    ...
    if (m_objType == DataObject::Type::FN_HANDLE)
    {
    ...
    else
    {
        ...
        if (m_sizeKnown)
        {
            ...
            // 
            infoStrs.push_back("Unknown size") ;
        }
        ...
        if (m_objType == DataObject::Type::CELLARRAY)
        {
        ...
        if (m_objType == DataObject::Type::STRUCTARRAY)
        {
            std::ostringstream stream;
            output += "\n" ;
            stream << getFields() ;
            output += indentText(stream.str() );
        }
    }
...
/*
    ...
    else
    {
        ...
        {
            // If the cell types do not match, the types do not match
            if (m_fields == other.m_fields) {} else { return false; }
        }
...
bool TypeInfo::operator < (const TypeInfo& other) const
{
    ...
    else
    {
        ...
        if (m_objType == DataObject::Type::STRUCTARRAY)
        {
            for (auto it = std::begin(m_fields) ;
                    it != std::end(m_fields) ; it++) {

                auto& other_field = other.m_fields ;
                auto other_itr = other_field.find(it->first) ;
                if (other_itr == std::end(other_field)) {
                    return true ;
                } else {
                    if (*it != *other_itr) {
                        return true ;
...
bool validateTypes(const DataObject* pObject, const TypeSet& typeSet)
{
    ...
    for (TypeSet::const_iterator typeItr = typeSet.begin(); typeItr != typeSet.end(); ++typeItr)
    {
        ...
        if (objType.getObjType() >= DataObject::Type::MATRIX_I32 && objType.getObjType() <= DataObject::Type::CELLARRAY)
        {
            ...
            if (objType.getObjType() == DataObject::Type::CELLARRAY)
            {
        ...
        else if (objType.getObjType() == DataObject::Type::FN_HANDLE)
        {
...
TypeSet typeSetReduce(const TypeSet& set)
{
    ...
    {
        // If the object type of this object does not match 
        // that of the current objects
        if (itr->getObjType() != curType || 
                itr->getObjType() == DataObject::Type::FN_HANDLE)
        {
        ...
        else
        {
            ...
            if (curType == DataObject::Type::CELLARRAY)
                cellTypes = typeSetUnion(cellTypes, itr->getCellTypes());

            if (curType == DataObject::Type::STRUCTARRAY) {
                bool both_have_the_same_fields_name = true ;
                 auto& other_fields = itr->getFields() ;
                for (auto it = std::begin(fields) ;
                        it != std::end(fields) ; 
                        it++) {
                    auto other_itr = other_fields.find(it->first) ;
                    if (other_itr == std::end(other_fields)) {
                        both_have_the_same_fields_name = false ;
                        break ;
                    }
                }
                //Verify both ways
                for (auto it = std::begin(other_fields) ;
                        it != std::end(other_fields) ; 
                        it++) {
                    auto exist = fields.find(it->first) ;
                    if (exist == std::end(fields)) {
                        both_have_the_same_fields_name = false ;
                        break ;
                    }
                }

                if (both_have_the_same_fields_name) {
                    for (auto it = std::begin(fields) ;
                            it != std::end(fields) ; it++) {
                        auto& other_field = itr->getFields() ;
                        auto other_itr = other_field.find(it->first) ;
                        assert (other_itr != std::end(other_field)) ;

                        if (!(*other_itr->second == * it->second )){
                            fields.erase(it->first) ;
                            TypeInfoPtr ptr_unknow_typeinfo = new TypeInfo() ;
                            ptr_unknow_typeinfo->setObjType(DataObject::Type::UNKNOWN); 
                            fields[it->first] = ptr_unknow_typeinfo;
                        }
                    }
                } else {
                    // Remove every field to say that we don't know anything
                    // about this structarray
                    PRDEBUG("Clearing the fields..\n");
                    fields.clear() ;
                }
            }
        }
...
TypeSetString multOpTypeMapping(const TypeSetString& argTypes)
{
    ...
    for (TypeSet::const_iterator type1 = argSet1.begin(); type1 != argSet1.end(); ++type1)
    {
        for (TypeSet::const_iterator type2 = argSet2.begin(); type2 != argSet2.end(); ++type2)
        {
            ...
            DataObject::Type objType;
            if (type1->getObjType() == DataObject::Type::MATRIX_C128 || type2->getObjType() == DataObject::Type::MATRIX_C128)
                objType = DataObject::Type::MATRIX_C128;
            else
                objType = DataObject::Type::MATRIX_F64;
...
TypeSetString minusOpTypeMapping(const TypeSetString& argTypes)
{
    ...
    for (TypeSet::const_iterator type1 = argSet1.begin(); type1 != argSet1.end(); ++type1)
    {
        // Determine the output type
        DataObject::Type objType;
        if (type1->getObjType() == DataObject::Type::MATRIX_C128)
            objType = DataObject::Type::MATRIX_C128;
        else
            objType = DataObject::Type::MATRIX_F64;
...
std::ostream& operator<<(std::ostream &strm, const TypeInfo &a) {
  return strm << a.toString() ;
}

std::ostream& operator<<(std::ostream &strm, const TypeSet &a) {
  strm << "Set {\n" ;
  std::ostringstream stream;
  for (auto it = a.cbegin() ; it != a.cend() ; ++it ) {
    stream << *it << "\n";
  }
  strm << indentText(stream.str() );
  strm << "}\n" ;
  return strm ;
}

std::ostream& operator<<(std::ostream &strm, const TypeSetMap &a) {
  for (auto it = a.cbegin() ; it != a.cend() ; ++it ) {
...
std::ostream& operator<<(std::ostream &strm, const TypeMap &a) {
  for (auto it = a.cbegin() ; it != a.cend() ; ++it ) {
    strm <<  "\"" << it->first << "\" " << *it->second << "\n" ;
  }
  return strm ;
...
/***************************************************************
* Function: isSuperior(a,b) return true if a > b 
* (is a compatible and bigger type)
* Purpose : Define the notion of "superiority" of typeinfo
* Initial : Matthieu Dubet on September 23, 2012
****************************************************************/
bool TypeInfo::isSuperior(const TypeInfo& a, const TypeInfo& b) {
        auto type_of_a = a.getObjType() ;

	if (type_of_a != b.getObjType() ) {
		return false ;
	}

        if (type_of_a == DataObject::Type::STRUCTARRAY) {
            // For each field in a, b should have the same 
            // field name with a superior type
            auto& fields_a = a.getFields() ;
            const TypeMap& fields_b = b.getFields() ;
            for (auto it = fields_b.cbegin() , end = fields_b.cend() ; it != end ; it++ ) {
                auto in_a = fields_a.find(it->first) ;
                if ( in_a ==  fields_a.end() ) { return false;}
                if ( ! TypeInfo::isSuperior( *in_a->second , *it->second)) { return false;}
            }
        }
