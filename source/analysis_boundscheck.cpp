...
bool operator<(const FlowFact &fact1, const FlowFact &fact2)
{
	...
	if (fact1.indexExpr->getExprType() == Expression::ExprType::INT_CONST )
	{
	...
	if (fact2.indexExpr->getExprType() == Expression::ExprType::INT_CONST )
	{
...
{
	return key1.indexExpr->getExprType() == Expression::ExprType::INT_CONST ? 
			
			key1.indexExpr->getExprType() == Expression::ExprType::INT_CONST && 
			key2.indexExpr->getExprType() == Expression::ExprType::INT_CONST &&
...
bool operator<(const IndexKey& key1, const IndexKey& key2)
{
	...
	if (key1.indexExpr->getExprType() == Expression::ExprType::INT_CONST )
	{
	...
	// build a unique string for indexkey1
	if (key2.indexExpr->getExprType() == Expression::ExprType::INT_CONST )
	{
...
{							
	...
	switch (expr->getExprType())
	{
		case Expression::ExprType::PARAM:
		{
			...
			{	
				if ( args[j]->getExprType() == Expression::ExprType::SYMBOL ||
						args[j]->getExprType() == Expression::ExprType::INT_CONST )
				{	
...
{	
	...
	for (FlowSet::const_iterator iter = t.begin(), iEnd = t.end(); iter != iEnd; ++iter)
	{
		...
		if ( (ff.matrixSymbol == symbol) && (ff.indexDim < size) 
				&& (ff.indexExpr->getExprType() == Expression::ExprType::INT_CONST) )	// valid known dimension
		{	
			// subscript value 
...
bool ArrayIndexAnalysis::searchCachedExpr(const BinaryOpExpr* bOpExpr, const SymbolExpr*& eqVar) const
{
  ...
  for (std::map<const SymbolExpr*, const BinaryOpExpr*>::const_iterator it = symbExprMap.begin(),
		 iEnd = symbExprMap.end(); it != iEnd; ++it)
	{
		...
		const SymbolExpr* eClass = dynamic_cast<const SymbolExpr*>(elOp->getExprType() == Expression::ExprType::SYMBOL ? elOp : erOp);
		const IntConstExpr* eConst = dynamic_cast<const IntConstExpr*>(elOp->getExprType() == Expression::ExprType::INT_CONST ? elOp : erOp);
		
		
		// if not, check the class of the DIV
		const Expression* lOp = bOpExpr->getLeftExpr();
		const Expression* rOp = bOpExpr->getRightExpr();
		const SymbolExpr* ivClass = dynamic_cast<const SymbolExpr*>(lOp->getExprType() == Expression::ExprType::SYMBOL ? lOp : rOp);
		const IntConstExpr* ivConst = dynamic_cast<const IntConstExpr*>(lOp->getExprType() == Expression::ExprType::INT_CONST ? lOp : rOp);
...
FlowSet ArrayIndexAnalysis::flowFunction(const Statement* stmt, const FlowSet& in)
{
	...
	switch(stmt->getStmtType())
	{
		case Statement::ASSIGN:
		{
			...
			if (lhs.size() == 1 && lhs[0]->getExprType() == Expression::ExprType::SYMBOL)
			{
	...
	switch (expr->getExprType())	
	{
		case Expression::ExprType::PARAM:
		{
			...
			for (size_t j = 0, size = args.size(); j < size; ++j)
			{
				...
				// generate others ... 
				if (  mustCheck && args[j]->getExprType() == Expression::ExprType::INT_CONST )
				{
...
{	
	// a binary expression?
	if (rhs->getExprType() == Expression::ExprType::BINARY_OP)
	{
...
inline bool ArrayIndexAnalysis::isBasicIV(const SymbolExpr* iSymbol, const BinaryOpExpr* bOpExpr) const
{
	...
	( (lOp->getExprType() == Expression::ExprType::INT_CONST || rOp->getExprType() == Expression::ExprType::INT_CONST ) );
}
...
inline bool ArrayIndexAnalysis::isDependentExpr(const BinaryOpExpr* bOpExpr, const SymbolExpr*& ivClass) const
{
	...
	ivClass = dynamic_cast<const SymbolExpr*>(lOp->getExprType() == Expression::ExprType::SYMBOL ? lOp : rOp);
	const IntConstExpr* ivConst = 
		dynamic_cast<const IntConstExpr*>( lOp->getExprType() == Expression::ExprType::INT_CONST ? lOp : rOp );
