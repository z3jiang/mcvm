...
ArrayObj* Interpreter::callFunction(Function* pFunction, ArrayObj* pArguments, size_t nargout)
{
	...
	try
	{
		...
		if (pFunction->isProgFunction())
		{
			...
			if (JITCompiler::s_jitEnableVar == true &&
			...
			)
			{
                            try {
                                ...
                                // Exit execution
                                std::cout << "Exception during compilation" << std::endl << ce.toString() << std::endl ;
                                return nullptr ;
                            }
			}
	...
	if (s_validateTypes.getBoolValue() == true)
	{
			...
			pStmt->getStmtType() != Statement::IF_ELSE)
		{
			...
			for (Expression::SymbolSet::const_iterator itr = stmtUses.begin(); itr != stmtUses.end(); ++itr)
			{
				...
				if (typeInfo.getObjType() >= DataObject::Type::MATRIX_I32 && typeInfo.getObjType() <= DataObject::Type::CHARARRAY)
				{
...
void Interpreter::evalAssignStmt(const AssignStmt* pStmt, Environment* pEnv)
{
	...
	if (pRightExpr->getExprType() == Expression::ExprType::PARAM)
	{
	...
	else if (pRightExpr->getExprType() == Expression::ExprType::SYMBOL)
	{
	...
	if (pResult->getType() == DataObject::Type::ARRAY)
	{
	...
	else
	{
		...
		assignObject(pLeftExpr, pResult->getType() == DataObject::Type::STRUCTARRAY ? pResult->copy() : pResult , pEnv, !pStmt->getSuppressFlag());
	}
}
...
void Interpreter::assignObject(const Expression* pLeftExpr, DataObject* pRightObject, Environment* pEnv, bool output)
{
	...
	switch (pLeftExpr->getExprType())
	{
      ...
      evalParamExpr(pParamExpr,pEnv,0, Expected(true,pRightObject)) ;
		...
		case Expression::ExprType::CELL_INDEX:
		{
			...
			if (pLeftObject->getType() != DataObject::Type::CELLARRAY)
			{
				// Throw an exception
    ...
    case Expression::ExprType::DOT:
    {
      ...
      evalDotExpr(pDotExpr,pEnv, Expected(true,pRightObject)) ;
    }
    break;
	...
	if (output)
	{
		...
		std::cout << pLeftExpr->toString() << " = "  << std::endl;

		// Display the value bound to the symbol
		std::cout << evalExpression(pLeftExpr,pEnv)->toString() << std::endl;
	}
}
...
ArrayObj* Interpreter::evalIndexArgs(const Expression::ExprVector& argVector, Environment* pEnv)
{
	...
	for (Expression::ExprVector::const_iterator itr = argVector.begin(); itr != argVector.end(); ++itr)
	{
		...
		if (pExpr->getExprType() == Expression::ExprType::RANGE)
		{
...
void Interpreter::evalExprStmt(const ExprStmt* pStmt, Environment* pEnv)
{
	...
	if (pExpr->getExprType() == Expression::ExprType::PARAM)
	{
	...
	else if (pExpr->getExprType() == Expression::ExprType::SYMBOL)
	{
    ...
    for (size_t i = 0 ; i < pArrayObj->getSize() ; i++ ) {
      std::cout << "ans = "  << std::endl << std::endl ;
      std::cout << pArrayObj->getObject(i)->toString() << std::endl ;
    }
...
Revisions and bug fixes:
*/
DataObject* Interpreter::evalExpression(const Expression* pExpr, Environment* pEnv, Expected expected)
{
	...
	switch (pExpr->getExprType())
	{
		...
		case Expression::ExprType::PARAM:
		{
			...
			DataObject* pResult = evalParamExpr((ParamExpr*)pExpr, pEnv, 1, expected );
		...
		case Expression::ExprType::SYMBOL:
		{
			...
			DataObject* pResult = evalSymbolExpr((SymbolExpr*)pExpr, pEnv, 1, expected);
		...
		{
			// Evaluate the lambda expression
			return evalDotExpr((DotExpr*)pExpr, pEnv, expected );
		}
		break;
...
DataObject* Interpreter::evalUnaryExpr(const UnaryOpExpr* pExpr, Environment* pEnv)
{
	...
	switch (pExpr->getOperator())
	{
		...
		case UnaryOpExpr::MINUS:
		{
			...
			if (pArgVal->getType() == DataObject::Type::MATRIX_C128)
			{
			...
			if (pArgVal->getType() != DataObject::Type::MATRIX_F64)
				pArgVal = pArgVal->convert(DataObject::Type::MATRIX_F64);
		...
		case UnaryOpExpr::NOT:
		{
			...
			if (pArgVal->getType() != DataObject::Type::LOGICALARRAY)
				pArgVal = pArgVal->convert(DataObject::Type::LOGICALARRAY);
		...
		case UnaryOpExpr::TRANSP:
		{
			...
			else if (pArgVal->getType() == DataObject::Type::MATRIX_F64)
			{
				// Get a typed pointer to the value
				MatrixF64Obj* pMatrix = (MatrixF64Obj*)pArgVal;
			...
			else if (pArgVal->getType() == DataObject::Type::MATRIX_C128)
			{
				// Get a typed pointer to the value
				MatrixC128Obj* pMatrix = (MatrixC128Obj*)pArgVal;
			...
			else if (pArgVal->getType() == DataObject::Type::CELLARRAY)
			{
				// Get a typed pointer to the value
...
DataObject* Interpreter::evalBinaryExpr(const BinaryOpExpr* pExpr, Environment* pEnv)
{
	...
	switch (pExpr->getOperator())
	{
		...
		case BinaryOpExpr::LEFT_DIV:
		{
			...
			if (pLeftVal->getType() == DataObject::Type::MATRIX_F64 && pRightVal->getType() == DataObject::Type::MATRIX_F64)
			{
		...
		case BinaryOpExpr::POWER:
		{
			...
			if (pLeftVal->getType() == DataObject::Type::MATRIX_C128 || pRightVal->getType() == DataObject::Type::MATRIX_C128)
			{
				// Convert the objects to 128-bit complex matrices, if necessary
				if (pLeftVal->getType() != DataObject::Type::MATRIX_C128)	 pLeftVal = pLeftVal->convert(DataObject::Type::MATRIX_C128);
				if (pRightVal->getType() != DataObject::Type::MATRIX_C128) pRightVal = pRightVal->convert(DataObject::Type::MATRIX_C128);
			...
			if (pLeftVal->getType() != DataObject::Type::MATRIX_F64) 	pLeftVal = pLeftVal->convert(DataObject::Type::MATRIX_F64);
			if (pRightVal->getType() != DataObject::Type::MATRIX_F64) pRightVal = pRightVal->convert(DataObject::Type::MATRIX_F64);
...
/***************************************************************
 * Function: Interpreter::evalDotExpr()
 * Purpose : Evaluate a dot expression (struct,oop)
 * Initial : Matthieu Dubet on March 12, 2012
 ****************************************************************
*/
DataObject* Interpreter::evalDotExpr(const DotExpr* pDotExpr, Environment* pEnv, Expected expected)
{
  Expression* pLeftExpr = pDotExpr->getExpr();

  StructArrayObj* expectedSAO = NULL ;

  if (expected.second != NULL) {
    expectedSAO = new StructArrayObj() ;
  }

  Expected recursiveExpected (false, expectedSAO ) ;

  DataObject* pObject = evalExpression(pLeftExpr, pEnv, recursiveExpected );

  if (pObject->getType() == DataObject::Type::STRUCTARRAY)
  {

    StructArrayObj* pSAO = (StructArrayObj*)pObject ;
    ScalarStruct* pScalarStruct = NULL ;

    if (pSAO->isEmpty() && expected.second != NULL ) {
      pScalarStruct = new ScalarStruct( ) ;
      StructArrayObj::writeElem1D(pSAO, 1, pScalarStruct) ;
    } else if ( pSAO->isScalar() ) {
      pScalarStruct = pSAO->getElem1D(1) ;
    } else {
      if ( expected.second != NULL) {
        throw RunError("Assigning multiple fields at the same time is forbidden") ;
      }
      //As Matlab, return only the first element
      ArrayObj * ao = new ArrayObj() ;
      for ( size_t i = 1 ; i <= pSAO->getNumElems() ; i++ ) {
        ScalarStruct* ss = pSAO->getElem1D(i) ;
        DataObject* d = NULL ;
        if ( ss != NULL) {
          d =  ( *ss) [ pDotExpr->getField() ] ;
        }
        if ( d != NULL) {
          ArrayObj::addObject(ao, d ) ;
        } else {
          ArrayObj::addObject(ao, new MatrixF64Obj ) ; // empty stuff
        }
      }
      return ao ;
      //pScalarStruct = StructArrayObj::readElem1D(pSAO,1) ;
      //if ( pScalarStruct == NULL ) {
        //pScalarStruct = new ScalarStruct( ) ;
      //}
    }

    assert (pScalarStruct != NULL) ;

    // Overwriting
    if (expected.first == true) {
      // Get original expr
       if (pLeftExpr->getExprType() == Expression::ExprType::PARAM ) {
         ParamExpr* pe = (ParamExpr*)pLeftExpr ;
         Expression* e = pe->getExpr() ;
         DataObject* pdo = evalExpression(e,pEnv) ;
         StructArrayObj* sao = (StructArrayObj*) pdo ;
         sao->m_Fields.insert( sao->m_Fields.begin() ,   pDotExpr->getField() ) ;
       } else {
         pSAO->m_Fields.insert( pSAO->m_Fields.begin() ,   pDotExpr->getField() ) ;
       }
      (*pScalarStruct) [pDotExpr->getField()]  =  expected.second ;
      return expected.second ;
    }

    DataObject* pFieldObject = (*pScalarStruct) [pDotExpr->getField()] ;

    // Exist without overwrite, just return it
    if ( pFieldObject != NULL ) {
      return pFieldObject ;
    }

    // Doesn't exist, try to assign the expected
    if (expected.second != NULL) {
       if (pLeftExpr->getExprType() == Expression::ExprType::PARAM ) {
         ParamExpr* pe = (ParamExpr*)pLeftExpr ;
         Expression* e = pe->getExpr() ;
    ...
    //for (auto it = pSAO->m_Fields.begin() ; it != pSAO->m_Fields.end(); it++ )
      //std::cout << " " << *it;

    if ( std::find( pSAO->m_Fields.begin(), pSAO->m_Fields.end(), pDotExpr->getField() ) == pSAO->m_Fields.end()) {
      throw RunError("unknown field '" + pDotExpr->getField() + "' in dot expression '" + pDotExpr->toString()  + "'") ;
    }
    
    // It exists but not in the current scalarstruct, means it should be empty
    //return new MatrixF64Obj() ;
    return NULL ;
  }
  // Otherwise
  ...
  {
    // Throw an exception
    throw RunError( "left expression ( here '" + pLeftExpr->toString() + "') in a dot expression should be a structarray (here '" +  pObject->getTypeName() + "')");
  }
}
/***************************************************************
...
Revisions and bug fixes:
*/
DataObject* Interpreter::evalParamExpr(const ParamExpr* pExpr, Environment* pEnv, size_t nargout, Expected expected)
{
	// Get a pointer to the symbol expression
	Expression* pLeftExpr = pExpr->getExpr();
  ...
  if (pLeftExpr->getExprType() == Expression::ExprType::SYMBOL ) {
    // Because of the call without argument stuff
	  pObject = evalSymbol((SymbolExpr*)pLeftExpr, pEnv, expected.second != NULL ? Expected(false,createBlankObj(expected.second->getType())) : Expected(false,NULL) );
  } else {
	  pObject = evalExpression(pLeftExpr, pEnv, expected.second != NULL ? Expected(false,createBlankObj(expected.second->getType())) : Expected(false,NULL) );
  }

  // If the left object is not a matrix
  if (expected.second != NULL && pObject->isMatrixObj() == false)
  {
    // Throw an exception
    throw RunError("unsupported left-expression type in parameterized assignment", pExpr);
  }


	// If the object is a function handle
	if (pObject->getType() == DataObject::Type::FN_HANDLE)
	{
	...
	if (pObject->getType() == DataObject::Type::FUNCTION)
	{
		...
		for (ParamExpr::ExprVector::const_iterator itr = argVector.begin(); itr != argVector.end(); ++itr)
		{
			...
			if (pExpr->getExprType() == Expression::ExprType::CELL_INDEX)
			{
	...
	else if (pObject->isMatrixObj())
	{
			...
			BaseMatrixObj* pLeftMatrix = (BaseMatrixObj*)pObject;
      ...
      // If the expected object is not a matrix
      if ( expected.second && (expected.second)->isMatrixObj() == false)
      {
        throw RunError("unsupported object type in parameterized assignment", pExpr);
      }

      BaseMatrixObj* pRightMatrix = (BaseMatrixObj*)expected.second;
      ...
      if (expected.first == true) {
        ...
        if (pRightMatrix->getType() == DataObject::Type::MATRIX_C128 && pLeftMatrix->getType() != DataObject::Type::MATRIX_C128)
        {
          // Convert the left matrix into a complex matrix
          //MatrixObj<> newMatrix = (BaseMatrixObj*)pLeftMatrix->convert(DataObject::Type::MATRIX_C128);
          //pLeftMatrix = newMatrix ;
        }

        // Set the matrix slice
        //StructArrayObj::writeElem1D( (StructArrayObj*)pLeftMatrix, 1, new ScalarStruct()) ;
        pLeftMatrix->setSliceND(pArguments, expected.second );

        return pLeftMatrix ;
      }

      BaseMatrixObj* pSubMatrix = pLeftMatrix->getSliceND(pArguments);
      pSubMatrix->m_Fields = pLeftMatrix->m_Fields ;
      return pSubMatrix ;
	}
...
DataObject* Interpreter::evalCellIndexExpr(const CellIndexExpr* pExpr, Environment* pEnv)
{
	...
	// If the object is not a cell array
	if (pObject->getType() != DataObject::Type::CELLARRAY)
	{
		// Throw an exception
...
Revisions and bug fixes:
*/
DataObject* Interpreter::evalSymbolExpr(const SymbolExpr* pExpr, Environment* pEnv, size_t nargout, Expected expected)
{
	// Evaluate the symbol
	DataObject* pResult = evalSymbol((SymbolExpr*)pExpr, pEnv, expected );

	// If the result is a function
	if (pResult->getType() == DataObject::Type::FUNCTION)
	{
...
Revisions and bug fixes:
*/
DataObject* Interpreter::evalSymbol(const SymbolExpr* pExpr, Environment* pEnv, Expected expected )
{

	// Lookup the symbol
	DataObject* pObject = Environment::lookup(pEnv, pExpr);
	
  if (pObject != NULL && pObject->getType() == DataObject::Type::FUNCTION && expected.second != NULL) {
    // we bind it over the function
		  Environment::bind(pEnv, pExpr , expected.second);
      return expected.second ;
  }
    ...
    // If we expect an object and it doesn't exist yet, we bind it and return it
    if (expected.second != NULL) {
		  Environment::bind(pEnv, pExpr , expected.second);
      return expected.second ;
    }
...
CompUnits Interpreter::loadMFile(const std::string& fileName, const bool bindScript)
{
	...
	nodes = CodeParser::parseSrcFile(fileName);
...
CompUnits Interpreter::loadSrcText(const std::string& srcText, const std::string& unitName, const bool bindScript)
{
	...
	nodes = CodeParser::parseSrcText(srcText);
...
void Interpreter::clearProgFuncs()
{
	...
	for (Environment::SymbolVec::iterator itr = symbols.begin(); itr != symbols.end(); ++itr)
	{
		...
		// If the object is a function
		if (pObject->getType() == DataObject::Type::FUNCTION && ((Function*)pObject)->isProgFunction())
		{
