...
void transformForLoop(ForStmt* pForStmt, StmtSequence::StmtVector& stmtVector, ProgFunction* pFunction)
{
	...
	assert (pLExpr->getExprType() == Expression::ExprType::SYMBOL);
	...
	if (pRExpr->getExprType() == Expression::ExprType::RANGE)
	{
		...
		if (pRangeExpr->getStepExpr()->getExprType() == Expression::ExprType::INT_CONST)
		{	
		...
		else if (pRangeExpr->getStepExpr()->getExprType() == Expression::ExprType::FP_CONST)
		{
			// Get the stepping constant value
