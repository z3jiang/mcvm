		...
		switch( pStmt->getStmtType() )
		{
			...
			{	
				...
				{	
					...
					if (rExprType == Expression::ExprType::CELL_INDEX)
					{
					...
					else if (rExprType == Expression::ExprType::PARAM) // function call?
					{
												  ...
												  const FlowContextPair& fcPair)
{
	...
	const SymbolExpr* rArrayVar = getSymFromArrayLValue(rhs, Expression::ExprType::CELL_INDEX);
	...
	{	
		...
		else if (lhsType == Expression::ExprType::PARAM)
		{
	...
	if (lhs != rhs) // ignore the trivial case; a = a
  	{ 
		...
		if (isArrayLValue(lhsType))
		{
			...
			{
				case Expression::ExprType::SYMBOL:		// a = b || a = c{1} || c{1} = b{1}
				...
				case Expression::ExprType::MATRIX:		// a = [x1, x2, x3; ...]
				case Expression::ExprType::RANGE:			// a = 1:1:10 ...
				{	
		...
		else if (lhsType == Expression::ExprType::PARAM)	// array or cell write
		{
...
{
	if (exprType == Expression::ExprType::SYMBOL)
	{
		return dynamic_cast<const SymbolExpr*>(lvalue);
	}
	else if (exprType == Expression::ExprType::CELL_INDEX)
	{
									   ...
									   CopyFlowSet& genSet)
{
	...
	{
	  if (arguments[i]->getExprType() != Expression::ExprType::SYMBOL) continue;
												  ...
												  const Environment* const pEnv) 
{
	...
	if (pObject && pObject->getType() == DataObject::Type::FUNCTION)
	{
