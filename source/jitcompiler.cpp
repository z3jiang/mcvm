...
void JITCompiler::initialize()
{
    ...
    // Create an execution engine for the module object
    // 	s_pExecEngine = llvm::ExecutionEngine::createJIT(s_pModule);
    s_pExecEngine =
    ...
    s_pFunctionPasses->add(llvm::createScalarReplAggregatesPass());
    ...
    regNativeFunc("StructArrayObj::makeScalar", (void*)StructArrayObj::makeScalar, VOID_PTR_TYPE, LLVMTypeVector(1, VOID_PTR_TYPE));
    regNativeFunc("makeScalarStructPtr", (void*)makeScalarStructPtr, VOID_PTR_TYPE, LLVMTypeVector());
    regNativeFunc("insertInScalarStruct", (void*)insertInScalarStruct, llvm::Type::getVoidTy(*s_Context), LLVMTypeVector(3,VOID_PTR_TYPE));
...
// For each input argument type set
    ...
    compVersion.inArgObjTypes.push_back(DataObject::Type::MATRIX_F64);
...
//	const ProgFunction::ParamVector& inParams = pFunction->getInParams();
    ...
    compVersion.outArgObjTypes.push_back(DataObject::Type::MATRIX_F64);
        ...
        CompVersion& compVersion, VariableMap& exitVarMap, const ProgFunction::ParamVector& outParams)
{
    ...
    for (size_t i = 0; i < outParams.size(); ++i)
    {
        ...
        else
        {
            ...
            llvm::Value* pNULLVal = llvm::ConstantInt::get(getIntType(PLATFORM_POINTER_SIZE), 0);
...
void JITCompiler::compileFunction(ProgFunction* pFunction, const TypeSetString& argTypeStr)
{
    ...
    variableMap[pNargoutSym] = Value(pNargoutVal, DataObject::Type::MATRIX_F64);

    // Add the "nargin" constant to the variable map
    variableMap[pNarginSym] = Value(llvm::ConstantInt::get(llvm::Type::getInt64Ty(*s_Context), argTypeStr.size()), DataObject::Type::MATRIX_F64);
...
Revisions and bug fixes:
*/
JITCompiler::CompVersion* JITCompiler::findFunction(ProgFunction* pFunction, ArrayObj* pArguments, size_t outArgCount)
{
...
)
{
    ...
    if (typeSet.size() != 1)
    {
        ...
        objType = DataObject::Type::UNKNOWN;
    ...
    if (typeInfo.isScalar() && objType != DataObject::Type::MATRIX_C128 && objType != DataObject::Type::CELLARRAY)
    {
        // If the value is a logical array
        if (objType == DataObject::Type::LOGICALARRAY)
        {
...
)
{
    ...
    else if (modeA == llvm::Type::getDoubleTy(*s_Context) || modeB == llvm::Type::getDoubleTy(*s_Context))
        return llvm::Type::getDoubleTy(*s_Context);

    // If either option is the int64 mode, return that option
    else if (modeA == llvm::Type::getInt64Ty(*s_Context) || modeB == llvm::Type::getInt64Ty(*s_Context))
        return llvm::Type::getInt64Ty(*s_Context);

    // If either option is the int32 mode, return that option
    else if (modeA == llvm::Type::getInt32Ty(*s_Context) || modeB == llvm::Type::getInt32Ty(*s_Context))
...
llvm::Value* JITCompiler::changeStorageMode(
    ...
    llvm::Value* pCurVal,
    DataObject::Type objType,
    const llvm::Type* newMode,
    const TypeInfo* current_typeinfo
)
{
    // Ensure that the variable is locally stored
    assert (pCurVal != NULL);

    // Get the current type for this llvm value
    auto current_type = pCurVal->getType() ;
    assert (pCurVal->getType() != NULL) ;

    assert (newMode != NULL && 
            "The storage mode conversion has to be a valid llvm::Type") ;

    // If no type conversion is needed
    if (current_type == newMode)
    {
    ...
    if (ConfigManager::s_verboseVar)
    {
        ...
        current_type->print(rso) ;
        std::cout << rso.str() <<  " ==> ";
        type_str.clear() ;
        newMode->print(rso);
        std::cout << rso.str() << " (" << 
            DataObject::getTypeName(objType) << ")" << std::endl;
    }
    ...
    if (pCurVal->getType() == llvm::Type::getInt1Ty(*s_Context))
    {
        ...
        else if (newMode == VOID_PTR_TYPE)
        {
            ...
            if (objType == DataObject::Type::MATRIX_F64)
            {
            ...
            else if (objType == DataObject::Type::CHARARRAY)
            {
            ...
            else if (objType == DataObject::Type::LOGICALARRAY)
            {
    ...
    else if (pCurVal->getType() == VOID_PTR_TYPE)
    {
        ...
        if (newMode == llvm::Type::getInt1Ty(*s_Context))
        {
            ...
            else if (objType == DataObject::Type::UNKNOWN)
            {
    ...
    // Test is the current llvm type is a structure
    // (it will be a pointer to a structure thus the first
    // isPointer() and then the actual isStruct()
    else if (current_type->isPointerTy() && 
            current_type->getPointerElementType()->isStructTy()) {

        assert(objType == DataObject::Type::STRUCTARRAY &&
               "Conversion from a llvm struct is only possible to a "
               "dataobject of type structarray") ;

        assert(newMode == VOID_PTR_TYPE &&
                "Conversion from a llvm struct required a pointer to a obj");

        assert(current_typeinfo != nullptr &&
                "To convert from an llvm struct, we need a typeinfo");

        // Dereference the initial pointer to the struct
        std::vector<llvm::Value*> gep_vec ;
        gep_vec.push_back
            (llvm::ConstantInt::get
             (llvm::Type::getInt64Ty(*s_Context), 0)) ;

        return comp_struct_obj_from_llvm_struct(
                irBuilder,
                gep_vec,
                pCurVal,
                *current_typeinfo) ;
...
)
{
    if (cpEntry.first)	// the symbol has not been moved
    {
        ...
        //if (mcvmVal.objType >=  DataObject::Type::MATRIX_I32 && mcvmVal.objType <= DataObject::Type::CELLARRAY)
...
)
{
    ...
    if (pExprStmt->getSuppressFlag() == true)
    {
        ...
        if (pExpr->getExprType() == Expression::ExprType::SYMBOL)
        {
        ...
        else if (pExpr->getExprType() == Expression::ExprType::PARAM)
        {
...
)
{
    ...
    if (pAssignStmt->getSuppressFlag() == false)
    {
        ...
        // Mark the symbols defined by the statement 
        // as written to the environment
    ...
    // Find the live variables after this statement
    LiveVarMap::const_iterator liveItr = 
        version.pLiveVarInfo->liveVarMap.find(pAssignStmt);
    assert (liveItr != version.pLiveVarInfo->liveVarMap.end());

    // Find the reaching definitions before this statement
    ReachDefMap::const_iterator defItr = 
        version.pReachDefInfo->reachDefMap.find(pAssignStmt);
    assert (defItr != version.pReachDefInfo->reachDefMap.end());

    // Find the type information before this statement
    TypeInfoMap::const_iterator typeItr = 
        version.pTypeInferInfo->preTypeMap.find(pAssignStmt);
    assert (typeItr != version.pTypeInferInfo->preTypeMap.end());
    ...
    // We compile SYMBOL and PARAM first because
    // we might need the nargout in case those represent 
    // a function call
    if (pRightExpr->getExprType() == Expression::ExprType::SYMBOL)
    {
    ...
    else if (pRightExpr->getExprType() == Expression::ExprType::PARAM)
    {
    ...
    else
    {
        ...
        {
            throw CompError(
                "the expression does not produce enough output values", 
                 pAssignStmt);
        }
    ...
    for (size_t i = 0; i < leftExprs.size(); ++i)
    {
        ...
        if (pLeftExpr->getExprType() == Expression::ExprType::SYMBOL)
        {
        ...
        else if (pLeftExpr->getExprType() == Expression::ExprType::PARAM)
        {
            ...
            for (ParamExpr::ExprVector::const_iterator argItr = arguments.begin(); argItr != arguments.end(); ++argItr)
            {
                ...
                if (storageMode != llvm::Type::getInt64Ty(*s_Context) && storageMode != llvm::Type::getDoubleTy(*s_Context))
                    argsScalar = false;
            }
            ...
            if (symObjType >= DataObject::Type::MATRIX_I32 && symObjType <= DataObject::Type::CHARARRAY &&
                symObjType != DataObject::Type::MATRIX_C128 && argsScalar &&
                rightVal.pValue->getType() != VOID_PTR_TYPE && rightVal.objType != DataObject::Type::MATRIX_C128 &&
        ...
        else if (pLeftExpr->getExprType() == Expression::ExprType::CELL_INDEX)
        {
            compAssignStmtFallback(
                    currentBuilder,
                    function,
                    version,
                    varMap,
        ...
        else if (pLeftExpr->getExprType() == Expression::ExprType::DOT)
        {

            /* The goal is to compile an assignment of an rhs value
             * to a dot expr.
             * Dot expr is recursive (left part can be a symbol, a param
             * expr, a dot expr..etc..)
             * Here we use the "compilable" map to determine what to do:
             *      - if the type of the root expr isn't changed before
...
//            auto root_expr = pDotExpr->getRootSymbol(); unused
            ...
            if (ConfigManager::s_verboseVar)
                std::cout << "Generating code for dot expr \"" << pDotExpr->toString() << "\" assignment" << std::endl;
        ...
        {
            // Throw an exception
            throw CompError("unsupported left-expression type in assignment",
                    pLeftExpr);
        }
    }
...
/***************************************************************
* Function: JITCompiler::compAssignStmtFallback()
* Purpose : Compile an assign statement falling back on the interpreter
* Initial : Matthieu Dubet on July 23, 2012
****************************************************************/

void JITCompiler::compAssignStmtFallback(
    ...
    Value rightVal
      ...
      std::cout << "FALLBACK" << std::endl ;
            ...
            writeVariables(builder, function, version, varMap, pExpr->getSymbolUses());

            // Create a call to perform the object assignment
            LLVMValueVector assignArgs;
            assignArgs.push_back(createPtrConst(pExpr));
            assignArgs.push_back(pRightObject);
            assignArgs.push_back(getCallEnv(function, version));
            assignArgs.push_back(llvm::ConstantInt::get(llvm::Type::getInt8Ty(*s_Context), 0));
            createNativeCall(
    ...
    ***********************/
    ...
    for (VariableMap::iterator mapItr = initExit.second.begin(); mapItr != initExit.second.end(); ++mapItr)
    {
        ...
        if (mapItr->second.pValue == NULL)
        {
            ...
            loopEntryVarMap[pSymbol] = Value(NULL, DataObject::Type::UNKNOWN);
        }
        else
        {
            ...
            assert (pPhiNode->getType() == VOID_PTR_TYPE || objectType != DataObject::Type::UNKNOWN);
        }
    }
...
)
{
    ...
    switch (pExpression->getExprType())
    {
        ...
        case Expression::ExprType::INT_CONST:
        {
            ...
            return Value(pConstValue, DataObject::Type::MATRIX_F64);
        }
        break;
...
)
{
    ...
    if (leftVal.pValue->getType() != VOID_PTR_TYPE && rightVal.pValue->getType() != VOID_PTR_TYPE )
    {
        ...
        DataObject::Type outType = boolOutput? DataObject::Type::LOGICALARRAY:DataObject::Type::MATRIX_F64;
    ...
    if (leftVal.pValue->getType() != VOID_PTR_TYPE)
    {
        ...
        if (leftVal.objType == DataObject::Type::LOGICALARRAY && rightVal.objType == DataObject::Type::LOGICALARRAY)
        {
            ...
            if (pLScalarFuncBool)
            {
                ...
                // Store the output object type
                outValue.objType = DataObject::Type::LOGICALARRAY;
            ...
            else
            {
                ...
                convArgs.push_back(getObjType(DataObject::Type::MATRIX_F64));
                ...
                rightVal.objType = DataObject::Type::MATRIX_F64;
            }
        }
        ...
        if (rightVal.objType == DataObject::Type::MATRIX_F64 && pLScalarFuncF64)
        {
            // Convert the left value to the f64 storage mode
            llvm::Value* pLeftVal = changeStorageMode(
                irBuilder,
            ...
            // Store the output object type
            outValue.objType = boolOutput? DataObject::Type::LOGICALARRAY:DataObject::Type::MATRIX_F64;
        ...
        if (rightVal.objType == DataObject::Type::CHARARRAY && pLScalarFuncChar)
        {
            // Convert the left value to the f64 storage mode
            llvm::Value* pLeftVal = changeStorageMode(
                irBuilder,
        ...
        if (rightVal.objType == DataObject::Type::MATRIX_C128 && pLScalarFuncC128)
        {
            // Convert the left value to the f64 storage mode
            llvm::Value* pLeftVal = changeStorageMode(
                irBuilder,
            ...
            // Store the output object type
            outValue.objType = boolOutput? DataObject::Type::LOGICALARRAY:DataObject::Type::MATRIX_C128;
        ...
        if (pLScalarFuncDef)
        {
            ...
            // Store the output object type
            outValue.objType = boolOutput? DataObject::Type::LOGICALARRAY:DataObject::Type::UNKNOWN;
    ...
    if (rightVal.pValue->getType() != VOID_PTR_TYPE)
    {
        ...
        if (leftVal.objType == DataObject::Type::LOGICALARRAY && rightVal.objType == DataObject::Type::LOGICALARRAY)
        {
            ...
            else
            {
                ...
                // Update the right object type
                leftVal.objType = DataObject::Type::MATRIX_F64;
            }
        }
        ...
        if (leftVal.objType == DataObject::Type::MATRIX_F64 && pRScalarFuncF64)
        {
        ...
        if (leftVal.objType == DataObject::Type::CHARARRAY && pRScalarFuncChar)
        {
            // Convert the right value to the f64 storage mode
            llvm::Value* pRightVal = changeStorageMode(
                irBuilder,
        ...
        if (leftVal.objType == DataObject::Type::MATRIX_C128 && pRScalarFuncC128)
        {
            // Convert the right value to the f64 storage mode
            llvm::Value* pRightVal = changeStorageMode(
                irBuilder,
    ...
    // Or if other cases did not apply
    {
        ...
        if (leftVal.objType == DataObject::Type::LOGICALARRAY && rightVal.objType == DataObject::Type::LOGICALARRAY && p2MatrixFuncBool)
        {
            // Create an object for the output value
            Value outValue;
        ...
        if (leftVal.objType == DataObject::Type::CHARARRAY && rightVal.objType == DataObject::Type::CHARARRAY && p2MatrixFuncChar)
        {
            // Create an object for the output value
            Value outValue;
        ...
        if ((leftVal.objType == DataObject::Type::MATRIX_F64 || rightVal.objType == DataObject::Type::MATRIX_F64) &&
            (leftVal.objType != DataObject::Type::MATRIX_C128 && rightVal.objType != DataObject::Type::MATRIX_C128) &&
            (leftVal.objType != DataObject::Type::UNKNOWN && rightVal.objType != DataObject::Type::UNKNOWN))
        {
            ...
            if (leftVal.objType != DataObject::Type::MATRIX_F64)
            {
                // Convert the left value to an f64 matrix
                LLVMValueVector convArgs;
                convArgs.push_back(leftVal.pValue);
            ...
            if (rightVal.objType != DataObject::Type::MATRIX_F64)
            {
                // Convert the left value to an f64 matrix
                LLVMValueVector convArgs;
                convArgs.push_back(rightVal.pValue);
        ...
        if ((leftVal.objType == DataObject::Type::MATRIX_C128 || rightVal.objType == DataObject::Type::MATRIX_C128) && p2MatrixFuncC128)
        {
            // If the left value is not a complex matrix
            if (leftVal.objType != DataObject::Type::MATRIX_C128)
            {
                // Convert the left value to a complex matrix
                LLVMValueVector convArgs;
                convArgs.push_back(leftVal.pValue);
                convArgs.push_back(getObjType(DataObject::Type::MATRIX_C128));
                leftVal.pValue = createNativeCall(
                    irBuilder,
                    (void*)DataObject::convertType,
                    convArgs
                ...
                // Update the right object type
                leftVal.objType = DataObject::Type::MATRIX_C128;
            }

            // If the right value is not a complex matrix
            if (rightVal.objType != DataObject::Type::MATRIX_C128)
            {
                // Convert the left value to a complex matrix
                LLVMValueVector convArgs;
                convArgs.push_back(rightVal.pValue);
                ...
                // Update the right object type
                rightVal.objType = DataObject::Type::MATRIX_C128;
            }
...
/***************************************************************
* Function: JITCompiler::compDotExpr()
* Purpose : Compile a dot  expression
* Initial : Matthieu Dubet on July 2012
****************************************************************
*/
JITCompiler::Value JITCompiler::compDotExpr(
    Statement* stmt,
    DotExpr* pDotExpr,
    CompFunction& function,
    CompVersion& version,
...
Revisions and bug fixes:
*/
JITCompiler::ValueVector JITCompiler::compFunctionCall(
        ...
        llvm::BasicBlock* pExitBlock)
{
  ...
  hotspot::Profiler::get()->instrumentFuncCall(
    ...
    )
    {
        ...
        std::vector<DataObject::Type> objTypes(nargout, DataObject::Type::UNKNOWN);
...
)
{
    ...
    else
    {
        ...
        else
        {
            ...
            value = Value(pValue, DataObject::Type::UNKNOWN);
        }
    }
...
)
{
    ...
    switch (matrixType)
    {
        ...
        case DataObject::Type::MATRIX_F64:
        {
        ...
        // Character array
        case DataObject::Type::CHARARRAY:
        {
            // Load the element array pointer
            llvm::Value* pDataPtr = loadMemberValue(
                currentBuilder,
        ...
        // Logical array array
        case DataObject::Type::LOGICALARRAY:
        {
            // Load the element array pointer
            llvm::Value* pDataPtr = loadMemberValue(
                currentBuilder,
...
)
{
    ...
    llvm::Value* pCompVal = entryBuilder.CreateICmpEQ(pObjType, getObjType(DataObject::Type::ARRAY));
...
llvm::Constant* JITCompiler::getObjType(DataObject::Type objType)
{
    ...
    return llvm::ConstantInt::get(getIntType(sizeof(DataObject::Type)), (int)objType);
}


llvm::Type* JITCompiler::generate_llvm_type_from_typeinfo(
        const TypeInfo& typeinfo)
{
    std::vector<llvm::Type*> ret ;
    switch(typeinfo.getObjType()) {
        case DataObject::Type::MATRIX_I32:
            return llvm::Type::getInt32Ty(*s_Context);
        case DataObject::Type::MATRIX_F64:
            return llvm::Type::getInt64Ty(*s_Context);
        case DataObject::Type::CHARARRAY:
            assert(false && "TODO") ;
        case DataObject::Type::STRUCTARRAY :
            {
                auto fields = typeinfo.getFields();
                for (auto it = std::begin(fields); it != std::end(fields) ; ++it) {
                    ret.push_back(
                            generate_llvm_type_from_typeinfo(*it->second));
                }
...
std::vector<llvm::Value*> JITCompiler::generate_llvm_gep_vector(
        TypeInfo& typeinfo, //modify it
        const Expression& exp)
{
    if (exp.getExprType() == Expression::ExprType::DOT) {
        auto dot_expr = static_cast<const DotExpr&>(exp);
        auto& left_expr = * dot_expr.getExpr() ;
        std::vector<llvm::Value*> rec_vector ;

        if (left_expr.getExprType() == Expression::ExprType::DOT) {
            // Call this recursively 
            rec_vector = generate_llvm_gep_vector(
                    typeinfo,
                    left_expr);
        }
        
        auto fields = typeinfo.getFields() ;
        auto field = dynamic_cast<const DotExpr&>(exp).getField();

        auto it = fields.find(field);
        assert (it != std::end(fields));

        // We add the offset
        auto indice = std::distance (fields.begin(), it);
        rec_vector.push_back(llvm::ConstantInt::get(llvm::Type::getInt32Ty(*s_Context), indice)) ;

        // Change typeinfo for the recursive call
        typeinfo = * it->second ;
...
llvm::Value* JITCompiler::comp_struct_obj_from_llvm_struct(
        llvm::IRBuilder<>& builder,
        const std::vector<llvm::Value*>& gep_vec,
        llvm::Value* val,
        const TypeInfo& typeinfo)
{

    assert (typeinfo.getObjType() == DataObject::Type::STRUCTARRAY &&
            " Generation of a struct obj need to have a struct typeinfo") ;

    auto scalar_struct = createNativeCall(
                            builder,
                            (void*)makeScalarStructPtr,
                            LLVMValueVector());

    auto fields = typeinfo.getFields() ;
    for (auto it = std::begin(fields) ; it != std::end(fields) ; ++it) {
        auto field_typeinfo = * it->second ;

        // Copy the previous gep vec
        // (they diverge for each field)
        std::vector<llvm::Value*> gep_vec_current = gep_vec ;

        // Add the indice for the current field
        auto indice = std::distance (fields.begin(), it);
        std::cout << "indice " << indice << std::endl ;
        gep_vec_current.push_back
            (llvm::ConstantInt::get
             (llvm::Type::getInt32Ty(*s_Context), indice)) ;


        llvm::Value* field_value ;
        // Switch on the expected type:
        //  - base class -> insert
        //  - recrusive case
        switch (field_typeinfo.getObjType()) {
            case DataObject::Type::MATRIX_F64: 
                {
                    ...
                    auto pointer = builder.CreateGEP(
                            val,
                            gep_vec_current,
                            it->first + "_conversion_ptr");
                    // Load the actual value
                    llvm::Value* loaded = builder.CreateLoad(pointer) ;
                    // Convert it to a pointer to a matrix
                    field_value = changeStorageMode(
                            builder,
                            loaded,
            ...
            case DataObject::Type::STRUCTARRAY :
                {
                    field_value = comp_struct_obj_from_llvm_struct(
                            builder,
                            gep_vec_current,
                            val,
                            field_typeinfo);
        ...
        // Convert the string field to a pointer to a C++ string
        // Hint : ConstantDataArray may be useful here
        //llvm::ConstantDataArray::getString(*s_Context,
        //it->first.c_str()),
        auto field_name_value = 
            builder.CreateGlobalStringPtr(it->first) ;

        LLVMValueVector make_args ;
        make_args.push_back(scalar_struct);
        make_args.push_back(field_name_value);
        make_args.push_back(field_value);

        createNativeCall(
                builder,
                (void*)insertInScalarStruct,
                make_args
                );
    }

    return createNativeCall(
            builder,
            (void*)StructArrayObj::makeScalar,
            LLVMValueVector(1,scalar_struct)) ;
}
