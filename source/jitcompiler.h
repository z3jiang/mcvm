...
private:
	...
	public:
		...
		// Default constructor
		Value()
		: pValue(NULL), objType(DataObject::Type::UNKNOWN) {}
	...
	typedef std::map<LibFuncKey, void*, std::less<LibFuncKey>> LibFuncMap;
...
public : static ArrayObj* callFunction(ProgFunction* pFunction, ArrayObj* pArguments, size_t outArgCount);
         static CompVersion* findFunction(ProgFunction* pFunction, ArrayObj* pArguments, size_t outArgCount) ;
...
private:
	...
	//typedef std::map<TypeSetString, CompVersion, std::less<TypeSetString>, gc_allocator<std::pair<TypeSetString, CompVersion> > > VersionMap;
        //FIXME GC
	typedef std::map<TypeSetString, CompVersion, std::less<TypeSetString>> VersionMap;
	...
	/**
	 * parse arguments and use necessary lookups to generate TypeSetString and
	 * determine whether # of arg is fixed
	 */
  static void extractArgTypeString(TypeSetString* retTypes, bool* retVarArgs,
	...
	// Method to change the storage mode of a value
        // FIXME : redundancy between objType and current_typeinfo
	static llvm::Value* changeStorageMode(
		llvm::IRBuilder<>& irBuilder,
		llvm::Value* pCurVal,
                DataObject::Type objType,
		const llvm::Type* newMode,
                const TypeInfo* current_typeinfo = nullptr
	);
  ...
  static void compAssignStmtFallback(
	...
	static llvm::BasicBlock* genLoopHeaderCopy(
        ...
        static llvm::Type* generate_llvm_type_from_typeinfo(
                const TypeInfo& typeinfo
                );
	
        static std::vector<llvm::Value*> generate_llvm_gep_vector(
                TypeInfo&,
                const Expression&
                );

        static llvm::Value* comp_struct_obj_from_llvm_struct(
		llvm::IRBuilder<>&,
