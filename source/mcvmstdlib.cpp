...
namespace mcvm_std_lib
{
	...
	*/
	...
	{	
		...
		if (pArguments->getSize() == 1 && pArguments->getObject(0)->isMatrixObj())
		{
			...
			if (pSizeArg->getType() != DataObject::Type::MATRIX_F64)
				pSizeMatrix = (MatrixF64Obj*)pSizeArg->convert(DataObject::Type::MATRIX_F64);
			else
	...
	*/	
	std::string formatPrint(ArrayObj* pArguments)
	{
		...
		if (pFormatArg->getType() != DataObject::Type::CHARARRAY)
			...
			if (thisChar == '%')
			{
				...
				switch (formatChar)
				{
					...
					case 's':
					{
						...
						if (pOutArg->getType() != DataObject::Type::CHARARRAY)
	...
	*/
	ArrayObj* absFunc(ArrayObj* pArguments)
	{
		...
		if (pArgument->getType() == DataObject::Type::MATRIX_C128)
		{
		...
		if (pArgument->getType() != DataObject::Type::MATRIX_F64)
			pArgument = pArgument->convert(DataObject::Type::MATRIX_F64);
	...
	*/
	ArrayObj* anyFunc(ArrayObj* pArguments)
	{
		...
		if (pMatrixArg->getType() == DataObject::Type::LOGICALARRAY)
		{			
			// Get a typed pointer to the argument
		...
		else if (pMatrixArg->getType() == DataObject::Type::MATRIX_F64)
		{
			// Get a typed pointer to the argument
	...
	*/
	ArrayObj* blkdiagFunc(ArrayObj* pArguments)
	{
		...
		for (size_t i = 0; i < pArguments->getSize(); ++i)
		{
			...
			if (pArg->getType() == DataObject::Type::MATRIX_C128)
		...
		for (size_t i = 0; i < pArguments->getSize(); ++i)
		{
			...
			if (pArg->getType() == DataObject::Type::MATRIX_F64)
				pMatrix = (MatrixF64Obj*)pArg;
			else
				pMatrix = (MatrixF64Obj*)pArg->convert(DataObject::Type::MATRIX_F64);
	...
	*/
	ArrayObj* bitwsandFunc(ArrayObj* pArguments)
	{
		...
		if (pLArg->getType() == DataObject::Type::MATRIX_F64 || pRArg->getType() == DataObject::Type::MATRIX_F64)
		{
	...
	*/
	ArrayObj* cdFunc(ArrayObj* pArguments)
	{
		...
		if (pArguments->getObject(0)->getType() != DataObject::Type::CHARARRAY)
	...
	*/
	ArrayObj* diagFunc(ArrayObj* pArguments)
	{
		...
		// If the argument is a 64-bit float matrix
		if (pArguments->getObject(0)->getType() == DataObject::Type::MATRIX_F64)
		{
	...
	*/
	ArrayObj* dispFunc(ArrayObj* pArguments)
	{
            ...
            auto obj = pArguments->getObject(0) ;
            assert (obj != NULL) ;
	...
	*/
	ArrayObj* dotFunc(ArrayObj* pArguments)
	{
		...
		if (pArg0->getType() == DataObject::Type::MATRIX_F64 && pArg1->getType() == DataObject::Type::MATRIX_F64)
		{			
	...
	*/
	ArrayObj* existFunc(ArrayObj* pArguments)
	{
		...
		// Ensure the argument is a character array
		if (pArg0->getType() != DataObject::Type::CHARARRAY)
			throw RunError("expected string argument");
	...
	*/
	ArrayObj* fevalFunc(ArrayObj* pArguments)
	{
		...
		if (pArguments->getObject(0)->getType() != DataObject::Type::FN_HANDLE)
	...
	*/
	ArrayObj* findFunc(ArrayObj* pArguments)
	{
		...
		// If the argument is a logical array
		else if (pArgument->getType() == DataObject::Type::LOGICALARRAY)
		{			
			// Get a typed pointer to the argument
	...
	*/
	ArrayObj* fopenFunc(ArrayObj* pArguments)
	{
		...
		if (pArg0->getType() != DataObject::Type::CHARARRAY || pArg1->getType() != DataObject::Type::CHARARRAY)
	...
	*/
	ArrayObj* fprintfFunc(ArrayObj* pArguments)
	{
		...
		if (pFirstArg->getType() == DataObject::Type::CHARARRAY)
		{
	...
	*/
	ArrayObj* iscellFunc(ArrayObj* pArguments)
	{
		...
		bool isCell = (pArguments->getObject(0)->getType() == DataObject::Type::CELLARRAY);
	...
	*/
	ArrayObj* isequalFunc(ArrayObj* pArguments)
	{
		...
		{
			// Ensure this argument is a matrix
			if (pArguments->getObject(i)->getType() != DataObject::Type::MATRIX_F64)
				throw RunError("arguments must be matrices");
	...
	*/
	ArrayObj* isnumericFunc(ArrayObj* pArguments)
	{
		...
		bool result = (objType == DataObject::Type::MATRIX_F64 || objType == DataObject::Type::MATRIX_C128);
	...
	*/
	ArrayObj* loadFunc(ArrayObj* pArguments)
	{
		...
		// Ensure the argument is a string
		if (pArgument->getType() != DataObject::Type::CHARARRAY)
			throw RunError("the format argument must be a string");
	...
	*/
	ArrayObj* lsFunc(ArrayObj* pArguments)
	{
		...
		for (size_t i = 0; i < pArguments->getSize(); ++i)
		{
			...
			if (pArg->getType() != DataObject::Type::CHARARRAY)
	...
	*/
	ArrayObj* notFunc(ArrayObj* pArguments)
	{
		...
		// If the argument is a logical array
		if (pArgument->getType() == DataObject::Type::LOGICALARRAY)
		{
			// Get a typed pointer to the argument
		...
		// If the argument is a matrix
		else if (pArgument->getType() == DataObject::Type::MATRIX_F64)
		{
			// Get a typed pointer to the argument
			MatrixF64Obj* pInMatrix = (MatrixF64Obj*)pArgument;
	...
	*/
	ArrayObj* num2strFunc(ArrayObj* pArguments)
	{
		...
		if (pArgument->getType() == DataObject::Type::MATRIX_F64 ||
			pArgument->getType() == DataObject::Type::MATRIX_C128 ||
			pArgument->getType() == DataObject::Type::LOGICALARRAY)
		{
	...
	*/
	ArrayObj* reshapeFunc(ArrayObj* pArguments)
	{
		...
		if (pSrcMatrix->getType() == DataObject::Type::MATRIX_F64)
		{
			// Get a typed pointer to the input matrix
	...
	*/
	...
	/***************************************************************
	* Function: createStructFunc()
	* Purpose : Create a structure
	* Initial : Matthieu Dubet on March 28, 2012
  *****************************************************************
	Revisions and bug fixes:
	*/
	ArrayObj* createStructFunc(ArrayObj* pArguments) {

		if (pArguments->getSize() == 0) {
		  return new ArrayObj(new StructArrayObj() );
    }

		if (pArguments->getSize() == 1) {
      ScalarStruct* ss = new ScalarStruct() ;
      StructArrayObj* sao = new StructArrayObj() ;
      StructArrayObj::writeElem1D(sao, 1,ss) ;
		  return new ArrayObj(sao) ;
    }

		if (pArguments->getSize() % 2 != 0)
			throw RunError("invalid argument count, il should be an even number") ;

    StructArrayObj* pSAO = new StructArrayObj () ;

    ScalarStruct* ss = new ScalarStruct() ;

    for (size_t i = 0 ; i < pArguments->getSize() ;) {
      DataObject* pArgKey = pArguments->getObject(i++);
      DataObject* pArgValue= pArguments->getObject(i++);

      if (pArgKey->getType() != DataObject::Type::CHARARRAY )
        throw RunError("invalid input argument types, field names should be a chararray");

      if (pArgValue->getType() == DataObject::Type::CELLARRAY )
        throw RunError("Multidimentionnal structure is not implemented yet");

      std::string key = ((CharArrayObj*)pArgKey)->getString();
      (* ss ) [key] = pArgValue ;
      pSAO->m_Fields.insert( pSAO->m_Fields.begin() , key ) ;
    }

    StructArrayObj::writeElem1D(pSAO,1,ss) ;
    return new ArrayObj (pSAO) ;
	}
	...
	*/
	ArrayObj* sumFunc(ArrayObj* pArguments)
	{
		...
		// If the argument is a 128-bit complex matrix
		if (pMatrixArg->getType() == DataObject::Type::MATRIX_C128)
		{
			// Get a typed pointer to the argument
		...
		// If the argument is not a 64-bit float matrix, convert its type
		if (pMatrixArg->getType() != DataObject::Type::MATRIX_F64)
			pMatrixArg = (BaseMatrixObj*)pMatrixArg->convert(DataObject::Type::MATRIX_F64);
	...
	*/
	ArrayObj* toeplitzFunc(ArrayObj* pArguments)
	{
		...
		if (pColObject->getType() == DataObject::Type::MATRIX_F64 && pRowObject->getType() == DataObject::Type::MATRIX_F64)
		{
			// Get typed pointers to the arguments
	...
	*/
	...
	LibFunction createStruct		("struct"	, createStructFunc	, nullTypeMapping			); //FIXME Need a proper type inference
	...
	*/
	void loadLibrary()
	{
		...
		Interpreter::setBinding(createStruct.getFuncName()	, (DataObject*)&createStruct		);
