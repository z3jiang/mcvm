...
****************************************************************
Revisions and bug fixes:
Guillaume Verdier: add throw() specifier
  log2 is also defined in cmath sometimes (it's part of the C99
  and POSIX standards) and has the throw() specifier, so
  compilers will complain if the declaration is different here.
*/
...
#define PRDEBUG(fmt, ...)
#else
#define PRDEBUG(fmt, ...) printf(fmt, ##__VA_ARGS__)
#endif
